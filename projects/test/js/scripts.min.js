...
$('.res_text .price_val').text($('#calc_main_remont_price').val() * $('#calc_main_square').val());
$('.remont_btn').click(function() {
    $('.remont_btn').removeClass("active");
    $(this).addClass("active");
    $('#calc_main_remont').val($(this).data('value'));

    $('#calc_main_remont_price').val($(this).data('price'));


    var price = get_price($('#calc_main_remont_price').val(), $('#calc_main_square').val());
    var discount = get_discount($('#calc_main_remont_price').val(), $('#calc_main_square').val());
    var discount_per = get_discount_percent($('#calc_main_square').val());
    $('.res_text .price_val').text(price);
    $('.res_text .discount_val').text(discount);

    $('.discount .discount_val').text(discount_per);

    $('#calc_main_remont_total').val(price);
    $('#calc_main_remont_discount').val(discount);
    $('#calc_main_remont_discount_percent').val(discount_per);

    switch (parseInt($("#calc_main_remont").val())) {
        case 613:
            res = get_days_cosmetic($('#calc_main_square').val());
            break;
        case 614:
            res = get_days_capital($('#calc_main_square').val());
            break;
        case 615:
            res = get_days_elit($('#calc_main_square').val());
            break;
    ...
    $('.res_text .time_val').text(res);
    $('#calc_main_remont_len').val(res);
});

function get_discount(price, square) {
    if (price == 0) {
        return "--";
    }
    k = 0;
    ...
    if (square >= 200)
        k = 0.15;
    return price * square * k;
}

function get_discount_percent(square) {
...
function get_price(price, square) {
    if (price == 0) {
        return "--";
    }
    k = 0;
    ...
    if (square >= 200)
        k = 0.15;
    return price * square * (1 - k);
}

function get_days_elit(val) {
    if (val < 40) return 60;
    if (val < 51) return 65;
    if (val < 61) return 70;
    if (val < 70) return 75;
    ...
    if (val < 80) return 85;
    if (val < 90) return 90;
    if (val < 100) return 95;
    if (val < 111) return 100;
    if (val < 120) return 105;
    if (val < 131) return 110;
    if (val < 141) return 115;
    if (val < 150) return 120;
    if (val < 160) return 125;
    if (val < 171) return 130;
    if (val < 181) return 135;
    if (val < 191) return 140;
    if (val < 201) return 145;
    if (val < 211) return 150;
    if (val < 221) return 155;
    if (val < 230) return 160;
    if (val < 241) return 165;
    if (val < 251) return 170;
    if (val < 260) return 175;
    if (val < 270) return 180;
    if (val < 281) return 185;
    if (val < 290) return 190;
    return 195;
}

function get_days_capital(val) {
    if (val < 40) return 50;
    if (val < 56) return 55;
    if (val < 61) return 60;
    if (val < 70) return 65;
    ...
    if (val < 80) return 75;
    if (val < 90) return 80;
    if (val < 100) return 85;
    if (val < 111) return 90;
    if (val < 120) return 95;
    if (val < 131) return 100;
    if (val < 141) return 105;
    if (val < 150) return 110;
    if (val < 160) return 115;
    if (val < 171) return 120;
    if (val < 181) return 125;
    if (val < 191) return 130;
    if (val < 201) return 135;
    if (val < 211) return 140;
    if (val < 221) return 145;
    if (val < 230) return 150;
    if (val < 241) return 155;
    if (val < 251) return 160;
    if (val < 260) return 165;
    if (val < 270) return 170;
    if (val < 281) return 175;
    if (val < 290) return 180;
    return 185;
}

function get_days_cosmetic(val) {
    return val * 0.5;
}
// $(function() {
//     $("#slider_bar").slider({
...
//             $("#calc_main_square").val(ui.value);
//             $("#calc_main .square_val").text(ui.value);
//             switch (parseInt($("#calc_main_remont").val())) {
//                 case 613:
//                     res = get_days_cosmetic(ui.value);
//                     break;
//                 case 614:
//                     res = get_days_capital(ui.value);
//                     break;
//                 case 615:
//                     res = get_days_elit(ui.value);
//                     break;
//                 default:
//                     res = "--";
//                     break;
...
//             $('#calc_main_remont_len').val(res);
//             res = get_price($('#calc_main_remont_price').val(), ui.value);
//             var disc_res = get_discount($('#calc_main_remont_price').val(), ui.value);
//             var disc_per = get_discount_percent(ui.value);

//             $('.discount .discount_val').text(disc_per);

//             $('.res_text .price_val').text(res);
//             $('#calc_main_remont_total').val(res);
//             $('#calc_main_remont_discount').val(disc_res);
//             $('#calc_main_remont_discount_percent').val(disc_per);
...
//     var data;
//     var name,email;
//     email = $('#calc_main_email').val();
//     name = $('#calc_main_name').val();
//     phone = $('#calc_main_phone').val();
//     if(email.length<5 || name.length<2  || phone.length<2  ||phone=="+7(___)___-__-__"){
//         swal({title: "", text: "Заполните необходимые поля!",   type: "error", confirmButtonText: "OK", confirmButtonColor: "#1F5107"});
//     }else{
//     data = $('#calc_main').serialize();
//     $.post("/calc/ajax.php",data,function(data){
//         //alert(data);
//     ga('send', 'event', 'Knopka', 'CALC_SEND');
//         swal({title: "", text: "Заявка отправлена! Файл со сметой будет выслан на указанную почту в течение 5 минут",   type: "success", confirmButtonText: "OK", confirmButtonColor: "#1F5107"});
//     });
// }
// });
$(function () {

$(function () {
	...
	const worksGallery = () => {
		...
		const works = Array.from(stage.children);
		const thumbCont = document.querySelector('.works__sideWrapper');
		const thumbs = Array.from(thumbCont.children);
		const nav = document.querySelector('.works__nav');
		const prevBtn = nav.querySelector('.works__slidePrev');
		const nextBtn = nav.querySelector('.works__slideNext');
		...
		if ( $(window).width() >= 768 ) {
			thumbs.forEach((thumb, index) => {
				thumb.addEventListener('click', evt => {
					thumbCont.querySelector('.current').classList.remove('current');
					thumb.classList.add('current');
					stage.querySelector('.current').classList.remove('current');
		...
		if ( $(window).width() < 768 ) {
			prevBtn.addEventListener('click', evt => {
				const currentWork = stage.querySelector('.current');
				const targetWork = currentWork.previousElementSibling;
				
				currentWork.classList.remove('current');
				targetWork.classList.add('current');
				checkNav(targetWork);
			});
			nextBtn.addEventListener('click', evt => {
				const currentWork = stage.querySelector('.current');
				const targetWork = currentWork.nextElementSibling;
	...
	$(".popup__form").submit(function() {
		$.ajax({
			...
			$(this).find("input").val("");
			$(this).hide();
			$('.popup__title').text('Спасибо за вашу заявку!');
			$('.popup__subtitle').text('Скоро мы с вами свяжемся');
